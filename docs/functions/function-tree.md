...
> 建筑工程实验室管理系统契约 BASE 树。F+I 级：I 级为全家族定标源，消费仓 ⊆ BASE（spec §3）。
> 消费仓在 F/I 级别镜像 BASE，可重标类型（FACING/接口）；交付列以 BASE 为准。
...
| M01.F05 | 认证管理 | OAuth 2.0 授权码 SSO（client_secret 后端持）+ lab 自家 JWT 签发，saas token 不出 lab 后端 | 已上线 |
...
| M05.F01 | 报告汇总 | 按报告类别输出试验报告汇总表 | 规划 |
